...
public:
    ...
    static const TYPE ZEROS = 0;
    static const TYPE ONES = (~ZEROS);
    static const unsigned int INF = (~0);
...
private:
    ...
    VT V;
    long long N; // size of given vector
    long long blocks;
    ...
    static void initializeStaticBlock(){
        if( bits.empty() ){
           ...
           /* cerr << "MOD_MODIFIER: " << toBinary(MOD_MODIFIER) << endl;
           cerr << "ONES: " << toBinary(ONES) << endl << "ZEROS: " << toBinary(ZEROS) << endl;
            
            for( int i=0; i<BLOCK_SIZE; i++ ) cerr << toBinary( bits[i] ) << endl;
            cerr << endl;
            for( int i=0; i<=BLOCK_SIZE; i++ ) cerr << toBinary(zerosOnes[i]) << endl;
            cerr << endl;
            for( int i=0; i<=BLOCK_SIZE; i++ ) cerr << toBinary(~zerosOnes[i]) << endl;
            cerr << "koniec" << endl;*/
        }
